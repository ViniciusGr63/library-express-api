import { criarEmprestimo, listarEmprestimos, buscarPorId, atualizarEmprestimo, deletarEmprestimo } from '../services/EmprestimoService.js';

class EmprestimoController {
    async create(req, res) {
        try {
            const emprestimo = await criarEmprestimo(req.body);
            res.status(201).json(emprestimo);
        } catch (error) {
            res.status(500).json({ error: 'Erro ao criar o empréstimo' });
        }
    }

    async getAll(req, res) {
        try {
            const emprestimos = await listarEmprestimos();
            res.status(200).json(emprestimos);
        } catch (error) {
            res.status(500).json({ error: 'Erro ao buscar empréstimos' });
        }
    }

    async getById(req, res) {
        try {
            const emprestimo = await buscarPorId(Number(req.params.id));
            if (emprestimo) {
                res.status(200).json(emprestimo);
            } else {
                res.status(404).json({ error: 'Empréstimo não encontrado' });
            }
        } catch (error) {
            res.status(500).json({ error: 'Erro ao buscar o empréstimo' });
        }
    }

    async update(req, res) {
        try {
            const emprestimo = await atualizarEmprestimo(Number(req.params.id), req.body);
            if (emprestimo) {
                res.status(200).json(emprestimo);
            } else {
                res.status(404).json({ error: 'Empréstimo não encontrado' });
            }
        } catch (error) {
            res.status(500).json({ error: 'Erro ao atualizar o empréstimo' });
        }
    }

    async delete(req, res) {
        try {
            const emprestimo = await deletarEmprestimo(Number(req.params.id));
            if (emprestimo) {
                res.status(200).json({ message: 'Empréstimo deletado com sucesso' });
            } else {
                res.status(404).json({ error: 'Empréstimo não encontrado' });
            }
        } catch (error) {
            res.status(500).json({ error: 'Erro ao deletar o empréstimo' });
        }
    }
}

export default new EmprestimoController();
